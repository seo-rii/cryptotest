...
백도어 스칼라 복원은 첫 telemetry 행에서 제거된 20비트를 열거한다. 후보마다 나머지 최대 5행을 상수 시간 modular arithmetic으로 검사하므로 시간은 `O(6 * 2^20)` modular operations이다. 이 인스턴스에서는 생존 후보가 하나라 실사용 추가 메모리는 `O(1)`이고, 현재 코드처럼 생존 후보를 목록에 모을 때의 이론적 최악 메모리는 `O(2^20)`이다.

상태 복원은 `r0`의 제거된 16비트를 열거한다. 각 후보에서 곡선식의 제곱근 존재 여부를 검사하고, 가능한 두 y 부호에 대해 약 86비트 스칼라 곱과 `r1`, `r2` 일치 여부를 확인한다. 따라서 상한은 `O(2^16 * log n)` 타원곡선 group operations이고 추가 메모리는 `O(1)`이다. 이는 부분군 전체에서 discrete logarithm을 푸는 것보다 압도적으로 작다. 순수 Python 구현은 이 환경의 반복 측정에서 약 21~34초가 걸렸다.

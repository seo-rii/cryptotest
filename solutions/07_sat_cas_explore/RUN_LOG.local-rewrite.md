# Challenge 7 historical SAT/CAS run log

> [!NOTE]
> This append-only log preserves experiments in chronological order.  The
> completed attack and verification are in the
> [final writeup](../../writeups/07_소인수분해.md); `current` and `next` below
> refer only to the date of each entry.
>
> [!WARNING]
> q-gap fields historically named `hard`, `proof`, or `hard_no_root` are
> heuristic.  The runs used `beta=0.5`, but the recovered smaller factor has
> `q^2 < N`, so the required `q >= N^beta` condition fails.  Positive margin
> and exhaustive completion sweeps do not make those rows sound clauses.  The
> low600 large-`p` oracle is distinct and remains conditional on its recorded
> Sage backend, bound, margin, and completion checks.

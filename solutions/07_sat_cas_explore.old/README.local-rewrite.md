> [!NOTE]
> This directory is an archived pre-solution snapshot.  Use the
> [final writeup](../../writeups/07_소인수분해.md) for the completed attack and
> the sibling [historical workspace](../07_sat_cas_explore/README.md) for the
> later experiment summary.  `Current` and `next` below retain their historical
> meaning.  Command paths are also preserved verbatim from before this snapshot
> was archived and are not guaranteed to run from this directory unchanged.
>
> [!WARNING]
> q-gap rows labelled `hard` or `proof` in the archived plan and log are
> heuristic: `beta=0.5` was used although the recovered `q` satisfies
> `q^2 < N`.  Exact solver contradictions and the large-`p` low600 oracle have
> different soundness conditions and must not be conflated with q-gap output.

# Challenge 7 archived SAT+CAS exploration

This folder is intentionally separate from the existing `solutions/try_07_*`
and lattice scripts.  It records the 2026-06-01 direction change: use a SAT
cube loop with external CAS/oracle checks, with the design goal of learning
only sound no-good clauses.

`EXPLORATION_PLAN.md` records the strategy at that checkpoint.  This README
lists the tooling; `RUN_LOG.md` keeps detailed experiment evidence.

## Files

- `EXPLORATION_PLAN.md`: historical control document for the then-active
  oracles, ranking-only directions, and exploration loop.

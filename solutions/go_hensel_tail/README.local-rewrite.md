> [!NOTE]
> Archived pre-solution prototype.  It uses the earlier `x1=p[210..248]`
> bit-range model recorded at that checkpoint and is not a final solver for the
> instance summarized in this repository.  Use the
> [final writeup](../../writeups/07_소인수분해.md) and
> [`solve_07_grouped_hm_flatter.cpp`](../solve_07_grouped_hm_flatter.cpp) for
> the completed attack.  Words such as `current`, `best`, and `next` below are

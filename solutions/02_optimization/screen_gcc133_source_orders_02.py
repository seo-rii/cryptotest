...
def run_screen(args: argparse.Namespace) -> dict[str, Any]:
    ...
    verifier_path = Path(__file__).with_name("verify_contest_candidate_02.c").resolve()
    ...
    with tempfile.TemporaryDirectory(prefix="challenge02-source-orders-") as name:
    ...
    return {
        "schema_version": 1,
        "experiment": "challenge02_gcc133_source_chain_order_screen",
        "container": {
            "image": IMAGE,
        ...
        "top_candidate": {
            "path": "solutions/02_optimization/contest_source_order_2103.c",
...
def main() -> None:
    ...
    parser.add_argument(
        "--json",
        type=Path,
        default=Path(__file__).with_name("gcc133_source_order_results_02.json"),
    )

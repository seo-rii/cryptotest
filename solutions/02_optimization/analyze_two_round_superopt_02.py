...
def official_vector_checks() -> dict[str, Any]:
    archive = REPOSITORY / "problems/2_암호구현.zip"
...
def compiler_codegen(
    ...
    probe_source = SCRIPT.with_name("two_round_chain_probe_02.c")
...
def main() -> None:
    ...
    report: dict[str, Any] = {
        "schema_version": 1,
        "analysis": "challenge02_two_round_scalar_superoptimization",
        "source": {
        ...
        "probe_source": {
            "path": "solutions/02_optimization/two_round_chain_probe_02.c",
            "sha256": hashlib.sha256(
                SCRIPT.with_name("two_round_chain_probe_02.c").read_bytes()
            ).hexdigest(),
        },
    ...
    if not args.skip_codegen:
        with tempfile.TemporaryDirectory(prefix="challenge02-chain-codegen-") as raw:
            temporary = Path(raw)
            report["codegen"] = {

...
def main() -> None:
    ...
    with ZipFile(root / "8_블록암호.zip") as archive:
    ...
    submitted_key = (root / "master_key.txt").read_text(encoding="ascii").strip()
    if submitted_key != key.hex():
        raise RuntimeError("master_key.txt is out of sync")

...
\hypersetup{
  ...
  linkcolor=blue,
  urlcolor=blue,
  pdftitle={8번 블록암호: 중간 상태 누출을 이용한 변형 AES 마스터 키 복원},
  pdfauthor={암호분석경진대회 제출 보고서},
  pdfsubject={Leak-assisted key recovery for a modified seven-round AES},
  pdfkeywords={AES, intermediate-state leak, key recovery, meet-in-the-middle}
}
\lstset{
...
\title{8번 블록암호\\중간 상태 누출을 이용한 변형 AES 마스터 키 복원}
...
여기서는 문제에 훨씬 강한 중간 상태 bit 누출과 특수한 라운드 키
permutation이 직접 주어진다. 원 공격에서 참고한 것은 중간 표현의
후보를 미리 계산하고 공통 값을 기준으로 결합한다는 관점뿐이다. 따라서
실제로 구현한 방법은 \textbf{leak-assisted bidirectional key-byte
constraint join}으로 구분한다. 이를 고전 Demirci--Selçuk 공격의
구현이나 성능 재현이라고 주장하지 않는다.
...
\texttt{complete\_partial\_keys()}는 한 partial key에서 처음 발견한
...
암호화해 해당 암호문과 일치하는 키만 남긴다. 서로 다른 생존 key가
정확히 하나인지 검사한 뒤에만 결과를 반환하므로, ``첫 해를 정답으로
간주''하는 조기 종료에 의존하지 않는다.
...
Python 3.11.2, Linux 6.1.0-51-cloud-amd64, glibc 2.36 환경의 같은
process에서 key 탐색, 완전 completion, 50,000쌍 검증을 함께 잰 두
실행은 41.706--57.160초, peak RSS 59,812--60,436 KiB
(약 58.4--59.0 MiB)였다. 이는 통제된 다회 benchmark가 아니며 Python
version, allocator, CPU 부하에 따라 달라진다. 따라서 위 후보 수는
공격 구조를 설명하는 값이고, wall time과 RSS는 현재 구현의 제한된
end-to-end 관측값일 뿐 복잡도의 증명이나 다른 장비의 성능 보장이 아니다.

\subsection{SMT probe의 비결론성}

탐색 초기에 임시 \texttt{z3-solver} 환경에서 full AES bit-vector
모델과 state/key layout 변형을 짧게 시도했으나 timeout에 해당하는
\texttt{unknown}으로 끝났다. 당시 probe code와 정확한 timeout 기록을
보존하지 않았으므로 이는 재현 가능한 실패 실험이 아니다. 특히
\texttt{unknown}은 UNSAT나 SMT 접근의 불가능성을 뜻하지 않는다. 최종
방법을 선택한 근거는 leak-assisted join이 실제 key를 복원하고 전체
50,000쌍으로 검증됐다는 양성 결과다.
...
\begin{itemize}
  ...
  \item 출력 key와 \texttt{submissions/08/master\_key.txt}의
        byte-for-byte 일치.
\end{itemize}
...
\begin{itemize}
  ...
  \item \href{https://www.iacr.org/archive/fse2008/50860106/50860106.pdf}
        ...
        구분했다.
  \item \path{problems/8_블록암호.zip}. 변형 7라운드 구조, 라운드 키
        permutation, 50,000쌍과 중간 상태 누출의 원문 자료다.
  \item \path{solutions/solve_08_aes_key.py}. 누출 completion,

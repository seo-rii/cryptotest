...
\title{네트워크보안}
\author{TLS 1.2 AES-GCM Nonce 재사용을 이용한 Record 위조}
\date{2026년 7월}
...
최종 제출 record는 77바이트이다. 아래 두 줄을 개행 없이 연결한 값을 제출한다.

\begin{lstlisting}
17030300489c269a9f29810ab0a99141d1d84c3df6bfdcdb7dec7cf30462db203fbc3
6a29772edbf159643b468f1f1e520e7341daee867a49665542cd9f01f5a9c906b73ba3e13bbfea0e98a18
\end{lstlisting}
...
AAD의 \texttt{0030}은 48바이트 평문 길이이고, record header의
\texttt{0048}은 explicit nonce(8)+ciphertext(48)+tag(16)=72바이트
wire fragment 길이이므로 서로 다르다. (GCM은 CBC와 달리 별도 padding이나
HMAC 없이 GHASH 내부에서만 zero padding을 쓴다.)
...
NIST GCM은 field element를 MSB-first로 표현하지만 다항식 연산은 bit 0을
상수항 계수로 쓰므로, 표현 경계에서 128비트 bit reverse를 적용해 다음
대응을 사용한다.
...
재사용 nonce에서 GCM authentication subkey $H$와 $S$를 모두 얻었으므로,
그 nonce에 대한 새 ciphertext와 tag를 만들 수 있다.

\section{평문 및 Tag 위조}

\subsection{Ciphertext 변경}

원래 평문은 문제 본문에 주어진 known plaintext다. 두 평문은 모두
48바이트이고 0-based offset 32의 한 바이트만 다르다.
...
구현 파일은 \texttt{solve\_tls\_gcm\_nonce\_reuse.py}다. Python 표준
...
이 제출 폴더의 \texttt{src} 디렉터리 안에서 다음을 실행한다.
...
\begin{itemize}
  ...
  \item 생성값과 \texttt{forged\_record.txt}의 일치.
\end{itemize}

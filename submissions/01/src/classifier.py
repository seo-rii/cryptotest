...
The classifier is a supervised logistic-regression model trained from the two
labelled ciphertext files.  Its features are invariant under a global Caesar
shift, but the evaluation deliberately remains a within-corpus, fixed-key
paired-line holdout; it does not claim generalisation to unseen corpora, key
lengths, or Vigenere keys.
"""
...
def matching_characters(left: str, right: str) -> int:
    if len(left) != len(right):
        raise ValueError("texts must have the same length")
...
def train_and_evaluate_classifier(
    ...
    """Evaluate with a paired line holdout inside the fixed corpus/key dataset."""
...
def load_ciphertexts(problem_zip: Path | None = None) -> tuple[list[str], list[str]]:
    if problem_zip is None:
        problem_zip = next(
            (
                parent / "problems" / "1_암호분석.zip"
                for parent in Path(__file__).resolve().parents
                if (parent / "problems" / "1_암호분석.zip").is_file()
            ),
            None,
        )
        if problem_zip is None:
            raise FileNotFoundError(
                "cannot locate problems/1_암호분석.zip; pass --problem-zip"
            )
    with ZipFile(problem_zip) as archive:
...
def main() -> None:
    ...
    parser.add_argument(
        "--problem-zip",
        type=Path,
        help="path to 1_암호분석.zip (auto-detected inside the repository)",
    )
    args = parser.parse_args()

    caesar_lines, vigenere_lines = load_ciphertexts(args.problem_zip)
    ...
    continuous_plaintext = vigenere_decrypt(ciphertext2, key)
    line_reset_matches = matching_characters(
        vigenere_plaintext, caesar_winner.plaintext
    )
    continuous_matches = matching_characters(
        continuous_plaintext, caesar_winner.plaintext
    )
    print(
        "Vigenere/plaintext agreement: "
        f"line-reset={line_reset_matches}/{len(ciphertext2)}, "
        f"continuous-key={continuous_matches}/{len(ciphertext2)}"
    )
    ...
    print(
        f"features={len(FEATURE_NAMES)} train={evaluation.train_count} "
        f"paired-within-corpus-held-out={evaluation.test_count}"
    )
    print(f"confusion(actual rows C,V; predicted columns C,V)={evaluation.confusion}")
    print(
        "paired within-corpus/fixed-key holdout accuracy="
        f"{evaluation.correct}/{evaluation.test_count} "
        f"({evaluation.correct / evaluation.test_count:.3%})"
    )
    predictions = []
    sample_shifts = []
    for number, sample in enumerate(CLASSIFIER_SAMPLES, 1):
        ...
        sample_shifts.append(sample_winner.shift)
        print(
    ...
    print("observable cipher structure (samples 1..4) = Caesar-like, Caesar-like, Caesar-like, Caesar-like")
    print(
        "likely intended constructor labels (samples 1..4) = "
        "Caesar, Vigenere, Vigenere, Caesar"
    )
    print(
        "intent inference: shifts 6 match the recovered Caesar key; shifts 10 "
        "match the first letter K of KLVOJ. This is not identifiable from the "
        "short ciphertexts alone."
    )
    ...
    assert line_reset_matches == 19_897
    assert continuous_matches == 3_076
    ...
    assert sample_shifts == [6, 10, 10, 6]

...
def main() -> None:
    ...
    with ZipFile(root / "5_동형암호.zip") as archive:
    ...
    if submitted_secret != secret:
        raise RuntimeError("02_secret_s.txt is out of sync")
    if submitted_state != list(state):
        raise RuntimeError("03_state.txt is out of sync")

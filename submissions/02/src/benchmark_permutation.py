...
def main() -> None:
    ...
    specifications = args.case or [
        "before=submissions/02/src/optimization/contest_before.c",
    ...
    archive = Path(__file__).resolve().parent / "2_암호구현.zip"
    candidate_verifier_source = (
        Path(__file__).resolve().parent / "optimization" / "verify_contest_candidate.c"
    )
    ...
    with tempfile.TemporaryDirectory(prefix="challenge-repeat-") as directory:
        ...
        oracle_compile = [
            ...
            "-Wextra",
            "-Wpedantic",
            str(Path(__file__).resolve().with_name("solve_permutation.c")),
            "-o",
            str(oracle),
        ]
    ...
    if args.json:
        ...
        protocol_paths = {
            "autotune_driver": (
                Path(__file__).resolve().parent / "optimization" / "autotune_255h.py"
            ),
            "benchmark_driver": Path(__file__).resolve(),
            "loop_audit": Path(__file__).resolve().with_name("loop_audit.py"),
            "reference_oracle": Path(__file__).resolve().with_name("solve_permutation.c"),
        ...
        report = {
            "schema_version": 5,
            "benchmark": "challenge_contest_shaped",
            "campaign_id": args.campaign_id,
            "environment": {

...
CASES = {
    ...
    "sse2": {
        "source": "submissions/02/src/optimization/contest_simd_sse2_twolanes.c",
        "cflags": [
            "-msse2",
            "-mno-avx",
            "-DCH2_SIMD_INLINE",
...
def hoist_benchmark_command(
    ...
    benchmark = repository / "submissions/02/src/benchmark_permutation.py"
    command = [
        sys.executable,
        str(benchmark),
        "--case",
...
def run_experiment(args: argparse.Namespace) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="challenge-simd-") as name:
    ...
    return {
        "schema_version": 2,
        "experiment": "challenge_lane_wise_simd_screen",
        "candidate_designs": {
            "avx2": (
...
def main() -> None:
    ...
    parser.add_argument(
        "--json",
        type=Path,
        default=Path(__file__).with_name("simd_results.json"),
    )

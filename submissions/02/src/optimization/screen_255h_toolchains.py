...
import screen_gcc133_layout as layout  # noqa: E402
import screen_gcc133_schedules as schedule  # noqa: E402
...
ARCHIVE = ROOT / "submissions/02/src/2_암호구현.zip"
VERIFIER = HERE / "verify_contest_candidate.c"
...
def verify_without_timing(
    ...
    results: dict[str, Any] = {}
    with tempfile.TemporaryDirectory(prefix="challenge-static-verify-") as name:
        temporary = Path(name)
...
def run_container_gcc_evidence(output: Path, random_cases: int) -> None:
    with tempfile.TemporaryDirectory(prefix="challenge-gcc-probes-") as name:
...
def run_gcc_evidence_container(
    ...
    command = [
        ...
        schedule.IMAGE,
        "python3",
        "submissions/02/src/optimization/screen_255h_toolchains.py",
...
def main() -> None:
    ...
    parser.add_argument(
        "--json",
        type=Path,
        default=Path(__file__).with_name("255h_toolchain_screen.json"),
    )
    parser.add_argument(
        "--container-gcc-evidence",
        type=Path,
    ...
    with tempfile.TemporaryDirectory(prefix="challenge-255h-screen-") as name:
        ...
        payload = {
            "schema_version": 1,
            "experiment": "challenge_core_ultra_7_255h_toolchain_static_followup",
            "scope": {

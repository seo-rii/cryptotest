...
BASE_SOURCE_RELATIVE = "submissions/02/src/optimization/contest_source_order_2103.c"
BASE_SOURCE = HERE / BASE_SOURCE_RELATIVE.rsplit("/", 1)[-1]
PORTABLE_SOURCE_RELATIVE = (
    "submissions/02/src/optimization/contest_inline_unrolled.c"
)
PORTABLE_SOURCE = HERE / PORTABLE_SOURCE_RELATIVE.rsplit("/", 1)[-1]
...
def build_result(args: argparse.Namespace) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(
        prefix="challenge-tenth-scalar-rotate-"
    ) as raw_temporary:
        ...
        return {
            "schema_version": 1,
            "experiment": "challenge_tenth_scalar_same_register_rotate",
            "scope": {
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=HERE / "tenth_codegen_scalar_rotate_results.json",
    )

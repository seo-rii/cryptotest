...
기존 18-candidate 원시 측정값은 [deep_results.txt](deep_results.txt),
새 인라인 캠페인은 [CPU 0 결과](inline_results.json),
[CPU 4 결과](inline_results_cpu4.json), 호출 경계별 분해는
[3-stage 결과](inline_stages_results.json), 추가 codegen과 loop 정렬은
[codegen 결과](inline_codegen_results.json)와
[alignment 결과](inline_alignment_results.json)에 보존했다. ROL 대조군은
[inline_rol_results.json](inline_rol_results.json), 실제 loop 감사는
[inline_assembly_audit.json](inline_assembly_audit.json)에 있다. GCC 13.3
실물 재현은 [gcc133_codegen_results.json](gcc133_codegen_results.json),
상수 배치의 결정적 검사는
[constant_placement_analysis.json](constant_placement_analysis.json)에
보존했다.
...
[`audit_inline.py`](audit_inline.py)는 `main`의 마지막 두 `clock()` 호출
...
16 네 개였다. 이 검사는
[`analyze_transform_lower_bound.py`](analyze_transform_lower_bound.py)로
재현하며 [구조화된 결과](transform_lower_bound.json)도 보존했다. 이는
...
[analyze_table_decomposition.py](analyze_table_decomposition.py)는 이
...
[inline_codegen_results.json](inline_codegen_results.json)에 있다.
...
[inline_alignment_results.json](inline_alignment_results.json)에 있다.
...
남았다. [재현 스크립트](screen_gcc133_schedules.py)와
[구조화된 결과](gcc133_schedule_screen.json)는 모든 후보 flag, binary/loop
...
[CPU 0](gcc133_schedule_results_cpu0.json)과
[CPU 4](gcc133_schedule_results_cpu4.json)에 있다.
...
[재배치 CPU 0](constant_reordering_results_cpu0.json),
[재배치 CPU 4](constant_reordering_results_cpu4.json),
[memory CPU 0](constant_memory_results_cpu0.json),
[memory CPU 4](constant_memory_results_cpu4.json)에 있다.

공식 상수 literal 특수화는 공식 vector만 통과하고 임의 상수 첫 사례에서
실패하므로 함수 계약을 위반한다. 더구나 핵심 연산 수도 줄지 않는다.
[`analyze_constant_placement.py`](analyze_constant_placement.py)는 원래
...
[`screen_gcc133_source_orders.py`](screen_gcc133_source_orders.py)는
...
[`gcc133_source_order_results.json`](gcc133_source_order_results.json)에
있다.

다음으로 [`screen_gcc133_layout.py`](screen_gcc133_layout.py)는 안정적인
...
109-build 결과는
[`gcc133_layout_screen.json`](gcc133_layout_screen.json)에 있다.
...
[`partial_unroll_results_cpu1.json`](partial_unroll_results_cpu1.json)과
[`partial_loop_confirm_cpu1.json`](partial_loop_confirm_cpu1.json)에 있다.

#### 제한된 grammar의 2-round superoptimization

한 word의 한 stage를
`T(x)=BSWAP64(ROL64(x,r) xor k)+a`로 두고
[`analyze_two_round_superopt.py`](analyze_two_round_superopt.py)에서 bit-vector
...
codegen은
[`two_round_superopt_results.json`](two_round_superopt_results.json)에
보존했다.
...
[`avx2_confirm_cpu1.json`](avx2_confirm_cpu1.json),
[`avx2_confirm_cpu3.json`](avx2_confirm_cpu3.json), 전체 12변형과 초기
CPU/SSE2 대조군은 [`simd_results.json`](simd_results.json)에 있다.
SSE2는 781-instruction/4,064-byte loop와 약 `0.40x`여서 기각했다.

마지막으로 [`screen_255h_toolchains.py`](screen_255h_toolchains.py)는 host
...
[`255h_toolchain_screen.json`](255h_toolchain_screen.json)에 있다. 따라서
...
[`screen_split_simd.py`](screen_split_simd.py),
[`split_simd_results.json`](split_simd_results.json)에 있다.

다음으로 [`screen_avx2_codegen.py`](screen_avx2_codegen.py)는 exact GCC
...
[`avx2_codegen_screen.json`](avx2_codegen_screen.json)에 있다.

마지막으로 CPU별 순위 역전이 timer·migration·standalone layout 때문인지
분리했다. [`benchmark_timing_stability.py`](benchmark_timing_stability.py)는
...
[`timing_stability_results.json`](timing_stability_results.json)에 보존하며
255H 성능 근거로 사용하지 않는다.

### 8차: 부분 register 고정, phase-stagger와 255H 공식 표

고정-register 실패를 더 좁혀 보면 문제는 inline assembly 자체가 아니라 동시에
살아 있는 YMM 값의 수였다. [`screen_avx2_inline_asm_alloc.py`](screen_avx2_inline_asm_alloc.py)는
...
[`avx2_inline_asm_alloc_results.json`](avx2_inline_asm_alloc_results.json)에
...
[`eighth_wave_timing_cpu1.json`](eighth_wave_timing_cpu1.json),
[`eighth_wave_timing_cpu3.json`](eighth_wave_timing_cpu3.json),
[`phase_staggered_results.json`](phase_staggered_results.json)에 있다.
...
추가 flag 쪽은 [`screen_gcc133_avx_flags.py`](screen_gcc133_avx_flags.py)가
...
[`gcc133_avx_flags_results.json`](gcc133_avx_flags_results.json)은 이 경계를
포함한 재현 가능한 결과다.

마지막으로 [`analyze_255h_instruction_model.py`](analyze_255h_instruction_model.py)는
...
[`instruction_model_255h.json`](instruction_model_255h.json)은 이 gap과
...
현재 schema 5는 [`solve_permutation.c`](../solve_permutation.c)의 별도
`--final-state N` 경로가 benchmark 초기 상태에서 reference 20-round 함수를 정확히
`N`회 합성해 예상 최종 상태를 계산한다. score-facing
[`benchmark_permutation.py`](../benchmark_permutation.py)는 semantic
...
[`test_benchmark_permutation.py`](test_benchmark_permutation.py)는 중복/누락
...
[`screen_avx2_commutative_layout.py`](screen_avx2_commutative_layout.py)는
...
[`avx2_commutative_layout_results.json`](avx2_commutative_layout_results.json)에
...
[`screen_avx2_pair_blocks.py`](screen_avx2_pair_blocks.py)와
[`avx2_pair_block_results.json`](avx2_pair_block_results.json)에 있다.
...
[`ninth_wave_timing_cpu1.json`](ninth_wave_timing_cpu1.json),
[`ninth_wave_timing_cpu3.json`](ninth_wave_timing_cpu3.json)에 있다. scalar
...
[`screen_tenth_avx2_counted_frontends.py`](screen_tenth_avx2_counted_frontends.py)는
...
[`tenth_avx2_counted_frontends_results.json`](tenth_avx2_counted_frontends_results.json)에
있다.

#### register 위치, 식 재결합과 scalar rotate 대조군

[`screen_tenth_codegen.py`](screen_tenth_codegen.py)는 이전 allocation
...
29-case 결과는 [`tenth_codegen_results.json`](tenth_codegen_results.json)에
있다.

scalar에서는 [`screen_tenth_codegen_scalar_rotate.py`](screen_tenth_codegen_scalar_rotate.py)가
...
따랐으며 결과는
[`tenth_codegen_scalar_rotate_results.json`](tenth_codegen_scalar_rotate_results.json)에
고정했다.
...
[`tenth_wave_timing_cpu1.json`](tenth_wave_timing_cpu1.json)과
[`tenth_wave_timing_cpu3.json`](tenth_wave_timing_cpu3.json)에 있으며,
...
[`stationarity_gate_timing_cpu3.json`](stationarity_gate_timing_cpu3.json)에
있으며 scalar incumbent는 그대로다.

#### 11차: 범위를 고정한 세 명령 AVX2 합성

마지막 로컬 알고리즘 축은
[`screen_eleventh_isa_synthesis.py`](screen_eleventh_isa_synthesis.py)로
...
[`eleventh_isa_synthesis_results.json`](eleventh_isa_synthesis_results.json)에
고정했다.
...
[`autotune_255h.py`](autotune_255h.py)는 `probe -> screen -> confirm ->
...
[deep_candidates.c](deep_candidates.c)는 18개 후보를 포함하며 다음 검사를
모두 통과했다.
...
[run_deep_review.py](run_deep_review.py)는 다음 절차를 사용한다.
...
```bash
python3 submissions/02/src/optimization/run_deep_review.py \
  --iterations 2000000 \
...
score-facing [`benchmark_permutation.py`](../benchmark_permutation.py)의
현재 schema 5는 reference oracle self-test와 candidate 검증을 분리한다. 각
`contest.c`를 `main`만 rename한 object로 만들고
[`verify_contest_candidate.c`](verify_contest_candidate.c)에 별도로
...
`autotune_candidates.json`은 이 후보와 portable/partial-unroll 대조군을

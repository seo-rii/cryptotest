...
/*
 * Pair words (0,3) and (1,2), but recompute each reversed dynamic constant
 * at its use site.  Four extra VPSHUFD operations per two-round pair shorten
 * invariant live ranges and test the cost of trading shuffles for spills.
 */
...
static inline __m128i swap_u64_lanes_recomputed(__m128i value) {
    __m128i swapped;
    __asm__ __volatile__("vpshufd $0x4e, %1, %0"
                         : "=x"(swapped)
                         : "x"(value));
    return swapped;
}
...
#define APPLY_TWO_ROUNDS_SPLIT_PAIR()                                        \
    do {                                                                      \
        ...
        outer = _mm_add_epi64(                                               \
            outer, swap_u64_lanes_recomputed(add_outer));                    \
        inner = _mm_add_epi64(                                               \
            inner, swap_u64_lanes_recomputed(add_inner));                    \
        outer = rotl64_lanes_xmm_avx2(outer, left_outer_reverse,              \
                                       right_outer_reverse);                   \
        inner = rotl64_lanes_xmm_avx2(inner, left_inner_reverse,              \
                                       right_inner_reverse);                   \
        outer = _mm_xor_si128(                                               \
            outer, swap_u64_lanes_recomputed(xor_outer));                    \
        inner = _mm_xor_si128(                                               \
            inner, swap_u64_lanes_recomputed(xor_inner));                    \
        outer = _mm_shuffle_epi8(outer, byte_swap);                           \
        inner = _mm_shuffle_epi8(inner, byte_swap);                           \
...
PERMUTE20_ATTRIBUTE static void permute_20rounds_unrolled(
    ...
    const __m128i left_outer_reverse =
        _mm_shuffle_epi32(left_outer, _MM_SHUFFLE(1, 0, 3, 2));
    const __m128i right_outer_reverse =
        _mm_shuffle_epi32(right_outer, _MM_SHUFFLE(1, 0, 3, 2));
    const __m128i left_inner_reverse =
        _mm_shuffle_epi32(left_inner, _MM_SHUFFLE(1, 0, 3, 2));
    const __m128i right_inner_reverse =
        _mm_shuffle_epi32(right_inner, _MM_SHUFFLE(1, 0, 3, 2));

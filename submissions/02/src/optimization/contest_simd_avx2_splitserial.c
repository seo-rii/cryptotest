...
/*
 * Process the word-reversal orbits (0,3) and (1,2) serially.  This tests
 * whether avoiding the simultaneous live ranges of two XMM chains removes
 * spills and offsets the lost instruction-level parallelism.
 */
...
#define APPLY_TWO_ROUNDS_SINGLE_XMM()                                        \
    do {                                                                      \
        value = rotl64_lanes_xmm_avx2(value, left_forward, right_forward);    \
        value = _mm_xor_si128(value, xor_forward);                            \
        value = _mm_shuffle_epi8(value, byte_swap);                           \
        value = _mm_add_epi64(value, add_reverse);                            \
        value = rotl64_lanes_xmm_avx2(value, left_reverse, right_reverse);    \
        value = _mm_xor_si128(value, xor_reverse);                            \
        value = _mm_shuffle_epi8(value, byte_swap);                           \
        value = _mm_add_epi64(value, add_forward);                            \
    } while (0)

#define PROCESS_SINGLE_XMM_CHAIN(first, second, rot_first, rot_second)        \
    do {                                                                      \
        __m128i value = _mm_set_epi64x((long long)state->w[(second)],         \
                                        (long long)state->w[(first)]);         \
        __m128i add_forward =                                                 \
            _mm_set_epi64x((long long)constants1[(second)],                   \
                            (long long)constants1[(first)]);                   \
        __m128i xor_forward =                                                 \
            _mm_set_epi64x((long long)constants2[(second)],                   \
                            (long long)constants2[(first)]);                   \
        const __m128i add_reverse = SWAP_U64_LANES(add_forward);              \
        const __m128i xor_reverse = SWAP_U64_LANES(xor_forward);              \
        const __m128i left_forward =                                          \
            _mm_set_epi64x((rot_second), (rot_first));                        \
        const __m128i right_forward =                                         \
            _mm_set_epi64x(64 - (rot_second), 64 - (rot_first));              \
        const __m128i left_reverse = SWAP_U64_LANES(left_forward);            \
        const __m128i right_reverse = SWAP_U64_LANES(right_forward);          \
        const __m128i byte_swap = _mm_setr_epi8(                              \
            7, 6, 5, 4, 3, 2, 1, 0, 15, 14, 13, 12, 11, 10, 9, 8);           \
        add_forward = keep_in_xmm_register(add_forward);                      \
        xor_forward = keep_in_xmm_register(xor_forward);                      \
        APPLY_TWO_ROUNDS_SINGLE_XMM();                                        \
        APPLY_TWO_ROUNDS_SINGLE_XMM();                                        \
        APPLY_TWO_ROUNDS_SINGLE_XMM();                                        \
        APPLY_TWO_ROUNDS_SINGLE_XMM();                                        \
        APPLY_TWO_ROUNDS_SINGLE_XMM();                                        \
        ...
        APPLY_TWO_ROUNDS_SINGLE_XMM();                                        \
        APPLY_TWO_ROUNDS_SINGLE_XMM();                                        \
        state->w[(first)] = (uint64_t)_mm_cvtsi128_si64(value);               \
        state->w[(second)] = (uint64_t)_mm_extract_epi64(value, 1);           \
    } while (0)
...
PERMUTE20_ATTRIBUTE static void permute_20rounds_unrolled(
    ...
    PROCESS_SINGLE_XMM_CHAIN(0, 3, 43, 14);
    PROCESS_SINGLE_XMM_CHAIN(1, 2, 7, 29);
}

#undef PROCESS_SINGLE_XMM_CHAIN
#undef APPLY_TWO_ROUNDS_SINGLE_XMM

...
This is an orchestration layer around ``benchmark_permutation.py``.  It
...
BENCHMARK = REPOSITORY / "solutions" / "benchmark_permutation.py"
LOOP_AUDIT = REPOSITORY / "solutions" / "loop_audit.py"
REFERENCE_ORACLE = REPOSITORY / "solutions" / "solve_permutation.c"
CANDIDATE_VERIFIER = SCRIPT.with_name("verify_contest_candidate.c")
PROBLEM_ARCHIVE = REPOSITORY / "problems" / "2_암호구현.zip"
DEFAULT_MANIFEST = SCRIPT.with_name("autotune_candidates.json")
...
def pinned_cpuid_probe(
    ...
    with tempfile.TemporaryDirectory(prefix="challenge-cpuid-") as directory:
...
def initialize_campaign_index(
    *,
    ...
    return {
        "schema_version": 1,
        "autotune": f"challenge_255h_{kind}",
        "created_at": utc_now(),
        "status": "RUNNING",
        "session": session,
...
def load_campaign_index(path: Path, expected_kind: str | None = None) -> dict[str, Any]:
    ...
    if expected_kind and kind != f"challenge_255h_{expected_kind}":
...
def command_decide(args: argparse.Namespace) -> None:
    ...
    decision: dict[str, Any] = {
        "schema_version": 1,
        "autotune": "challenge_255h_decision",
        "created_at": utc_now(),
        "status": status,
        "policy": args.policy,
        "incumbent": baseline,

...
def main() -> None:
    ...
    parser.add_argument(
        "--source",
        type=Path,
        default=SOLUTIONS.parent / "contest.c",
        help="contest source (default: the submission's contest.c, one directory above src/)",
    )
    parser.add_argument(
        "--portable-source",
        type=Path,
        default=Path(__file__).resolve().with_name("contest_inline_unrolled.c"),
    ...
    with tempfile.TemporaryDirectory(prefix="challenge-asm-audit-") as directory:
    ...
    if args.json:
        ...
        output = {
            "schema_version": 2,
            "audit": "challenge_main_timing_loop",

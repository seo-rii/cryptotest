...
def run_q_middle_gap_coppersmith(
    *,
    ...
    small_roots_backend: str = "auto",
) -> dict[str, Any]:
    if small_roots_backend not in {"auto", "sage", "fpylll_linear"}:
        raise ValueError("small_roots_backend must be auto, sage, or fpylll_linear")
    ...
    base_report: dict[str, Any] = {
        **bound_report,
        ...
        "small_roots_backend_requested": small_roots_backend,
        "cache_key": [
            parts["low_bits"],
            parts["prefix_start"],
            hex(parts["q_lo"]),
            ...
            try:
                result_queue.put(
                    run_q_middle_gap_coppersmith(
                        ...
                        small_roots_backend=small_roots_backend,
                    )
                )
    ...
    try:
        if small_roots_backend == "fpylll_linear":
            raise ImportError("forced fpylll_linear backend")
        from sage.all import PolynomialRing, Zmod, ZZ
    except Exception as sage_exc:  # pragma: no cover - depends on local Sage packaging
        if small_roots_backend == "sage":
            return {
                "status": "unavailable",
                **base_report,
                "elapsed_seconds": time.time() - started,
                ...
                "roots_sample": [],
                "factors": [],
                "failure_reason": f"Sage backend unavailable: {sage_exc}",
                "hard_clause_eligible": False,
                "no_root_hard_clause_eligible": False,
                "small_roots_backend": "unavailable",
            }
        try:
            import sympy as sp
            from fpylll import IntegerMatrix, LLL

            root_bound = 1 << parts["gap_bits"]
            monic_const = (parts["const"] * pow(1 << parts["low_bits"], -1, n)) % n
            m = int(ceil(max(COPPERSMITH_BETA * COPPERSMITH_BETA / epsilon, 7 * COPPERSMITH_BETA)))
            t = int(floor(m * (1 / COPPERSMITH_BETA - 1)))
            f_powers: list[list[int]] = [[1]]
            for _ in range(m):
                previous = f_powers[-1]
                current = [0] * (len(previous) + 1)
                for index, coefficient in enumerate(previous):
                    current[index] += coefficient * monic_const
                    current[index + 1] += coefficient
                f_powers.append(current)

            rows: list[list[int]] = []
            for index in range(m + 1):
                scale = n ** (m - index)
                rows.append([coefficient * scale for coefficient in f_powers[index]])
            for shift in range(1, t + 1):
                rows.append([0] * shift + f_powers[m])

            max_degree = max(len(row) for row in rows) - 1
            root_bound_powers = [1]
            for _ in range(max_degree):
                root_bound_powers.append(root_bound_powers[-1] * root_bound)

            matrix = IntegerMatrix(len(rows), max_degree + 1)
            for row_index, row in enumerate(rows):
                for degree, coefficient in enumerate(row):
                    matrix[row_index, degree] = int(coefficient * root_bound_powers[degree])

            LLL.reduction(matrix)
            symbol = sp.Symbol("y")
            root_values_set: set[int] = set()
            for row_index in range(matrix.nrows):
                coefficients: list[int] = []
                divisible = True
                for degree in range(matrix.ncols):
                    value = int(matrix[row_index, degree])
                    scale = root_bound_powers[degree]
                    if value % scale != 0:
                        divisible = False
                        break
                    coefficients.append(value // scale)
                if not divisible or all(coefficient == 0 for coefficient in coefficients):
                    continue
                polynomial = sp.Poly.from_list(list(reversed(coefficients)), gens=symbol, domain=sp.ZZ)
                for root in polynomial.ground_roots():
                    if root.q != 1:
                        continue
                    root_value = int(root)
                    if 0 <= root_value < root_bound:
                        root_values_set.add(root_value)

            root_values = sorted(root_values_set)
            ...
            return {
                ...
                **base_report,
                ...
                "factors": factors,
                "failure_reason": None if factors else f"fpylll small_roots returned no valid divisor; Sage import failed: {sage_exc}",
                "hard_clause_eligible": False,
                "no_root_hard_clause_eligible": False,
                "small_roots_backend": "fpylll_linear",
                "coppersmith_m": m,
                "coppersmith_t": t,
                "lattice_dimension": [matrix.nrows, matrix.ncols],
            }
            ...
            return {
                ...
                **base_report,
                ...
                "roots_sample": [],
                "factors": [],
                "failure_reason": f"Sage import failed: {sage_exc}; fpylll fallback failed: {fallback_exc}",
                "hard_clause_eligible": False,
                "no_root_hard_clause_eligible": False,
                "small_roots_backend": "unavailable",
            }

...
def run_low_coppersmith(
    ...
    x = ring.gen()
    old_handler = None
    timer_enabled = timeout_seconds is not None and timeout_seconds > 0

    def handle_timeout(signum, frame):
        raise TimeoutError("low Coppersmith small_roots timed out")
...
def main() -> int:
    ...
    if args.timeout_seconds < 0:
        raise SystemExit("--timeout-seconds must be nonnegative")

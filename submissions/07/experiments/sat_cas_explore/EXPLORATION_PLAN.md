# Challenge 7 Corrected-PDF Plan

The active instance is the corrected PDF uploaded on 2026-06-05.  The old
exploration folder is preserved as `../sat_cas_explore.old`; do not mix its
old-mask ledgers into this workspace.
...
1. Freeze the x7-focused low600 q-gap default loop.  It fixes
   ...
   repeated 146-literal independent drops are useful branch-exclusion evidence,
   but they have not shown convergence toward `p` or `q`.  Do not continue with
   seed-base 20260767 by default.
...
12. Keep the older fully fixed low600 p-Coppersmith ledgers as sound supporting
   evidence, not as the next broad batch.  They prove branch exclusions, but the
   146-literal clauses have not produced useful convergence by themselves.
13. Treat p two-sided Coppersmith and residual partial-p/Coron/HM attempts as
   ...
   become hard clauses unless separately proved by the q-gap/low600 criteria.

## Literature-Backed Experiment Map

The new plan is organized by applicability to this corrected instance:
...
Fresh corrected q-gap batches closed 192 ranked hard-line candidates:

```text
split hash A: 64 candidates, gap 456, 64 hard no-roots, no factor
split hash B: 64 candidates, gap 456, 64 hard no-roots, no factor
low42 hash A: 64 candidates, gap 462, 64 hard no-roots, no factor
```

This proves the fresh runner and oracle plumbing.  It is not enough coverage to
claim a solve ETA.  Next work should not be another blind q-gap ledger batch.
...
It gives q-gap about 407-408 bits and a large hard margin.  The first two
cubes both returned hard no-root and independently dropped `150:4`, `920:4`,
and `784:6`.  Loading those two ledgers moves the next model to `265:84=3`.
Use `run_fullx1x5_drop_loop.py` for further one-cube-at-a-time exploration:

```bash
python3 cryptotest/submissions/07/src/sat_cas_explore/run_fullx1x5_drop_loop.py \
  --iterations 4 \
  --resume-jsonl tmp/ct07_fresh_fullx1x5_first_cube_drops.jsonl \
  --resume-jsonl tmp/ct07_fresh_fullx1x5_second_cube_drops.jsonl
```

For q-gap 408, use `epsilon=0.04`.  It keeps about 21 bits of effective hard
margin and makes the loop roughly 35-40x faster than `epsilon=0.02`.

`run_fullx1x5_drop_loop.py` supports two drop modes:

```text
--drop-mode independent  # default; one learned clause per droppable window
--drop-mode cumulative   # tests the growing union of drop windows
```

The cumulative smoke test on the all-zero cube verified `150:4` followed by
`920:4` as a sound combined drop.  It produced one 130-literal clause from the
138 selected literals, using 273 q-gap oracle calls and 8 workers.
...
`150:4, 920:4`.  The all-zero cube proved a 116-literal clause from 124
selected literals.  One follow-up cube with 16 workers took about 108 seconds,
so 8 workers remains the better default unless the host has substantially more
free CPU.

For the same hi32 all-zero cube, these independent byte windows have also been
proved droppable:
...
They are safe hard clauses, but they do not move the SAT model very far by
themselves.  The first substantial 16-bit cumulative union,
`265:8 + 273:8`, is now complete in
`tmp/ct07_fresh_hi32_union_265_273_shard0_eps0028`: all 128 shards passed,
covering `65536 / 65536` hard no-root completions with no factor.  The produced
`learned_clause.jsonl` is a 108-literal hard clause for the high32 cube and
should be loaded into the next SAT-ledger step.

The next comparable proof target is the high-side union `798:8 + 806:8`, or a
SAT-ledger run that loads the new `learned_clause.jsonl` and asks for the next
high32 cube before spending another multi-hour union proof.  Use
`run_q_gap_union_shards.py` rather than a monolithic `semi_programmatic_sat.py`
call for future 16-bit unions.  Resume checks a proof key to avoid mixing
shards from different q-gap parameters.

That SAT-ledger follow-up has now closed four more q-gap hard no-root cubes
after the completed `265:8 + 273:8` union clause and the earlier `x0=1` ledger.
The closed `150:4` values are `2`, `3`, `7`, and `5`; all used
`265:84=0`, `798:32=0`, and `920:4=0`.  Each cube had `q_gap_bits=437`,
generated two independent byte-drop clauses for `265:8` and `273:8`, and found
no factor.  Loading all current ledgers selects `150:4=4` as the next sample
cube, so the low x0 front is not exhausted yet.

The later high32 follow-ups closed that low x0 front fully for
`265:84=0, 798:32=0, 920:4=0`: all `150:4 = 0..15` are now hard no-root, with
no factor.  The next high32 sample moves to `265:84=65536`, which means this
shape is now walking x1 space.  Do not keep spending most runtime there unless
a stronger x1 generalization is added.

The stronger medium drop set is now:

```text
150:4, 920:4, 265:8, 273:8, 784:8, 792:8
```

It costs about 1057 q-gap calls per cube and currently runs around
2 minutes/cube with 8 workers.  It skips much farther than the small default
set.  A 3-iteration run after all-zero byte-drop ledgers moved through
`265:84 = 520, 778, 1794`, with every drop passing.

A later 5-iteration continuation on this q-gap 408 medium-drop line closed
`265:84 = 1292, 1028, 1542, 3337, 2317`, again all hard no-root and no factor.
The next 10-iteration continuation closed
`265:84 = 3088, 2072, 2830, 2567, 3587, 3851, 6405, 6927, 7953, 7442`, again
all hard no-root and no factor.  The next sample is `265:84=5911`.  This
remains the preferred near-term SAT-ledger line because it advances the x2
frontier much faster than the high32 x1-walk.

A diversified q-gap hard-line hit-first batch
...
all hard no-root and no factor.  The next sample is `265:84=7196`.

The p-window `[362,830)` oracle is not yet a good broad batch primitive: a
single current sample timed out at 120 seconds with `epsilon=0.005`.  The
corrected seven-variable unknown-divisor lattice preflight also remains
negative-margin.  Another q-gap 408 continuation closed
`265:84 = 7196, 7710, 14370, 14625, 15392, 15652, 10533, 11564, 11304, 10281`,
again all hard no-root and no factor.  The next sample is `265:84=12589`.

Do not spend a broad batch on outside-only q-gap 504 yet.  It only fixes
...
Direct OR-Tools CP-SAT is also not a current primary path.  A corrected
all-zero edge branch with p decision priority on `265:84` and `784:46` returned
`UNKNOWN` after about 66 seconds.

The low600 cumulative line should not be the next primary batch.  Its first
post-cumulative continuation stalled for more than 5 hours before producing a
valid cube row.  Keep existing low600 ledgers as sound proof evidence, but do
not spend broad runtime there until the minimization/oracle timeout behavior is
fixed.

The corrected q-gap 456 hit-first line remains a sound oracle, but blind
hash-diversified sampling is not enough by itself:
...
On the corrected mask, fixing `150:4` and `920:4` already opens the low-q
frontier to 265 bits and gives the high-q prefix near 830/831.  Full `x6` plus
48 bits from one side gives a hard q-gap oracle.  The first 128 ranked
candidates across `x2_low48` and `x5_high48` were checked in 369 seconds:
all `128/128` were hard no-root, no roots, no factor.  Expand this search with
larger diversified beams and then add clause/minimization accounting around the
same q-gap condition.

The first wider continuation checked 256 additional hash-diversified q-gap 456
candidates in 784 seconds, again all hard no-root and no factor.  That brings
the corrected q-gap 456 evidence to 384 sampled branches with no hit.  This
confirms the oracle is stable, but also shows that hash-diversified hit-first
sampling is not enough by itself: the 102-bit hard frontier is too large unless
another exact ranker, SAT loop, or residual lattice narrows the branch first.

The best current SAT-ledger continuation is now the q-gap 408 full-x1/full-x5
line with cumulative edge-nibble minimization:
...
The latest probe, 3-iteration continuation, and 20-iteration continuation
proved that `150:4` and `920:4` can be dropped together for every tested cube:
each clause drops 8 bits and keeps 130 literals, with `256/256` combined
completions hard no-root.  This is strictly stronger than the earlier
independent-drop ledgers and currently runs around 30-50 seconds per cube with
8 workers.  The cumulative resume manifest is:
...
```bash
python3 cryptotest/submissions/07/src/sat_cas_explore/run_projection_frontier_batches.py \
  --iterations 1 \
...
python3 cryptotest/submissions/07/src/sat_cas_explore/run_cube_representative_minimization.py \
...
```bash
python3 cryptotest/submissions/07/src/run_tail_cp_sat_edge_sweep.py \
...
```bash
python3 cryptotest/submissions/07/src/sat_cas_explore/run_edge_minimization_queue.py \
...
```bash
python3 cryptotest/submissions/07/src/sat_cas_explore/branch_partial_coppersmith.py \
...
```bash
python3 cryptotest/submissions/07/src/sat_cas_explore/rank_q_gap_assumption_pairs.py \
...
```text
Deep-prefix scorer:
  script: cryptotest/submissions/07/src/sat_cas_explore/rank_deep_prefix_candidates.py
...
```text
New sampler:
  cryptotest/submissions/07/src/sat_cas_explore/sample_diverse_edge_completions.py
...
4. The selected-cube scorer is implemented:
     cryptotest/submissions/07/src/sat_cas_explore/score_selected_cube_samples.py
...
5. The scored free batch loop is implemented:
     cryptotest/submissions/07/src/sat_cas_explore/run_pwindow420_scored_batches.py
   Smoke run:
...
10. Projection novelty sampler:
   ...
   ```bash
   python3 cryptotest/submissions/07/src/sat_cas_explore/build_projection_frontier.py \
   ...
   python3 cryptotest/submissions/07/src/sat_cas_explore/sample_diverse_edge_completions.py \
   ...
   python3 cryptotest/submissions/07/src/sat_cas_explore/run_ranked_q_gap_direct.py \
...
17. representative no-x7 minimization automation:
   ...
   ```text
   cryptotest/submissions/07/src/sat_cas_explore/run_cube_representative_minimization.py
   ```
...
20. no-x7 direct/cumulative cycle runner:
   ...
   ```bash
   python3 cryptotest/submissions/07/src/sat_cas_explore/run_nox7_cumulative_cycle.py \

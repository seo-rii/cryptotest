...
def evaluate_low_coppersmith_completion_task(task: dict[str, object]) -> dict[str, object]:
    instance = task["instance"]
    fixed_ranges = task["fixed_ranges"]
    cube_ranges_without_window = task["cube_ranges_without_window"]
    completion_bits = task["completion_bits"]
    completion_value = int(task["completion_value"])
    low_bits = int(task["low_bits"])
    low_coppersmith_epsilon = float(task["low_coppersmith_epsilon"])
    low_coppersmith_min_hard_margin_bits = float(task["low_coppersmith_min_hard_margin_bits"])
    low_coppersmith_oracle_timeout_seconds = float(task["low_coppersmith_oracle_timeout_seconds"])
    ...
    if not all_bits_known(completion_mask, 0, low_bits):
        return {
            "completion_value": completion_value,
            "status": "not_triggered_after_completion",
            "oracle_called": False,
            "hard_eligible": False,
            "factors": [],
        }
    ...
    completion_report = run_low_coppersmith(
        ...
        epsilon=low_coppersmith_epsilon,
        min_hard_margin_bits=low_coppersmith_min_hard_margin_bits,
        timeout_seconds=(
            low_coppersmith_oracle_timeout_seconds
            if low_coppersmith_oracle_timeout_seconds > 0
            else None
        ),
    )
    completion_status = str(completion_report.get("status"))
    return {
        ...
        "hard_eligible": (
            completion_status == "no_roots"
            and bool(completion_report.get("hard_clause_eligible"))
        ),
        "factors": completion_report.get("factors") or [],
        "report": completion_report,
    }
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Z3 solver timeout for each cube-selection check; 0 disables the guard",
    )
    ...
    parser.add_argument("--low-coppersmith-minimize-workers", type=int, default=1)
    ...
    parser.add_argument(
        "--q-gap-cumulative-drop-window",
        action="append",
        default=[],
        help=(
            "with --q-gap-independent-drop-clauses, first verify these START:WIDTH "
            "windows cumulatively and add one generalized learned-clause variant"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=parse_fixed_range,
        help=(
            "temporary p-bit assumption START:WIDTH:VALUE for cube selection; "
            "assumed bits are included in emitted cube_ranges so learned clauses stay guarded"
        ),
    )
    ...
    if args.low_coppersmith_minimize_workers < 1:
        raise SystemExit("--low-coppersmith-minimize-workers must be positive")
    ...
    q_gap_cumulative_drop_windows: list[tuple[int, int]] = []
    for raw_window in args.q_gap_cumulative_drop_window:
        try:
            start_text, width_text = raw_window.split(":", 1)
        except ValueError as exc:
            raise SystemExit("--q-gap-cumulative-drop-window must be START:WIDTH") from exc
        drop_start = int(start_text, 0)
        drop_width = int(width_text, 0)
        if drop_start < 0 or drop_width <= 0:
            raise SystemExit(
                "--q-gap-cumulative-drop-window must have nonnegative start and positive width"
            )
        if (1 << drop_width) > args.q_gap_minimize_max_completions:
            raise SystemExit(
                "--q-gap-cumulative-drop-window exceeds --q-gap-minimize-max-completions"
            )
        q_gap_cumulative_drop_windows.append((drop_start, drop_width))
    ...
    cube_assumption_values: dict[int, int] = {}
    for assumption_range in args.cube_assume_p_range:
        for offset in range(assumption_range.width):
            bit = assumption_range.start + offset
            value = (assumption_range.value >> offset) & 1
            if bit < 0 or bit >= instance.p_bits:
                raise SystemExit("--cube-assume-p-range bit is outside p")
            if (base_mask >> bit) & 1:
                if ((base_known >> bit) & 1) != value:
                    raise SystemExit(
                        f"--cube-assume-p-range conflicts with fixed p bit {bit}"
                    )
                continue
            previous = cube_assumption_values.get(bit)
            if previous is not None and previous != value:
                raise SystemExit(f"conflicting --cube-assume-p-range value for bit {bit}")
            cube_assumption_values[bit] = value
    ...
    selected_bits = sorted(set(selected_bits) | set(cube_assumption_values))
    ...
    cube_assumptions = [
        ...
        for bit, value in sorted(cube_assumption_values.items())
        if bit in bit_vars
    ]
    ...
    counters = {
        ...
        "low_coppersmith_minimize_workers": args.low_coppersmith_minimize_workers,
        ...
        "q_gap_coppersmith_cumulative_drop_clauses": 0,
        "q_gap_coppersmith_cumulative_dropped_literals": 0,
    ...
    q_gap_coppersmith_cache: dict[tuple[int, int, int, int, float, float], dict[str, object]] = {}
    low_coppersmith_mask = (1 << args.low_coppersmith_bits) - 1
    while counters["cubes"] < args.max_cubes:
        result = solver.check(*cube_assumptions)
        if result != z3.sat:
            print(
                json.dumps(
                    {
                        "event": "solver_done",
                        "status": str(result),
                        "cube_assumption_count": len(cube_assumptions),
                        **counters,
                    },
                    sort_keys=True,
                ),
                ...
                if q_gap_bits > args.q_gap_max_bits:
                    ...
                    if (
                        ...
                        if args.q_gap_independent_drop_clauses:
                            ...
                            if q_gap_cumulative_drop_windows:
                                dropped_q_gap_bits: set[int] = set()
                                for drop_start, drop_width in q_gap_cumulative_drop_windows:
                                    ...
                                    if args.q_gap_minimize_workers > 1 and completion_count > 1:
                                        ...
                                        with concurrent.futures.ProcessPoolExecutor(
                                            ...
                                            completion_results = list(
                                                executor.map(evaluate_q_gap_completion_task, tasks)
                                            )
                                        for completion_result in completion_results:
                                            ...
                                            if completion_status == "factored":
                                                event["q_gap_coppersmith_minimization_factor"] = (
                                                    completion_result.get("report", completion_result)
                                                )
                                                print(json.dumps(event, sort_keys=True), flush=True)
                                                print(
                                                    json.dumps(
                                        ...
                                        for completion_value in range(completion_count):
                                            ...
                                            if completion_status == "factored":
                                                ...
                                                print(
                                                    json.dumps(
                                                        {"event": "factored", **completion_report},
                                                        sort_keys=True,
                                                    ),
                                                    flush=True,
                                                )
                                ...
                                if dropped_q_gap_bits:
                                    learned_clause_variants.append(
                                        {
                                            "scope": "cumulative_minimized_q_gap_selected_bits",
                                            "drop_windows": [
                                                {"start": start, "width": width}
                                                for start, width in q_gap_cumulative_drop_windows
                                            ],
                                            "dropped_bits": sorted(dropped_q_gap_bits),
                                            "dropped_literal_count": len(dropped_q_gap_bits),
                                            "literal_count": len(selected_q_gap_bits) - len(dropped_q_gap_bits),
                                        }
                                    )
                            ...
                            if learned_clause_variants:
                                ...
                                variant_scopes = sorted(
                                    {str(variant.get("scope")) for variant in learned_clause_variants}
                                )
                                event["learned_clause_scope"] = (
                                    variant_scopes[0]
                                    if len(variant_scopes) == 1
                                    else "mixed_minimized_q_gap_selected_bits"
                                )
                                ...
                                cumulative_variants = [
                                    variant
                                    for variant in learned_clause_variants
                                    if variant.get("scope") == "cumulative_minimized_q_gap_selected_bits"
                                ]
                                independent_variants = [
                                    variant
                                    for variant in learned_clause_variants
                                    if variant.get("scope") == "independent_minimized_q_gap_selected_bits"
                                ]
                                counters["q_gap_coppersmith_cumulative_drop_clauses"] += len(
                                    cumulative_variants
                                )
                                counters["q_gap_coppersmith_cumulative_dropped_literals"] += sum(
                                    int(variant["dropped_literal_count"]) for variant in cumulative_variants
                                )
                                counters["q_gap_coppersmith_independent_drop_clauses"] += len(
                                    independent_variants
                                )
                                counters["q_gap_coppersmith_independent_dropped_literals"] += sum(
                                    int(variant["dropped_literal_count"]) for variant in independent_variants
                                )
                            ...
                            if cumulative_rows:
                                event["q_gap_coppersmith_cumulative_minimization"] = cumulative_rows
        ...
        if (
            ...
            if low_report is None:
                ...
                low_report = run_low_coppersmith(
                    ...
                    timeout_seconds=(
                        args.low_coppersmith_oracle_timeout_seconds
                        if args.low_coppersmith_oracle_timeout_seconds > 0
                        else None
                    ),
                )
            ...
            if (
                ...
                for drop_start, drop_width in low_drop_windows:
                    ...
                    factors: list[object] = []
                    all_no_roots = True
                    if args.low_coppersmith_minimize_workers > 1 and completion_count > 1:
                        tasks = [
                            {
                                "instance": instance,
                                "fixed_ranges": fixed_ranges,
                                ...
                                "low_coppersmith_min_hard_margin_bits": (
                                    args.low_coppersmith_min_hard_margin_bits
                                ),
                                "low_coppersmith_oracle_timeout_seconds": (
                                    args.low_coppersmith_oracle_timeout_seconds
                                ),
                        ...
                        max_workers = min(args.low_coppersmith_minimize_workers, completion_count)
                        if args.low_coppersmith_oracle_timeout_seconds > 0:
                            completion_results = []
                            wave_timeout = max(
                                args.low_coppersmith_oracle_timeout_seconds + 30.0,
                                args.low_coppersmith_oracle_timeout_seconds * 1.25,
                            )
                            for wave_start in range(0, len(tasks), max_workers):
                                wave_tasks = tasks[wave_start : wave_start + max_workers]
                                wave_results: list[dict[str, object]] = []
                                executor = concurrent.futures.ProcessPoolExecutor(
                                    max_workers=len(wave_tasks)
                                )
                                future_tasks = {
                                    executor.submit(
                                        evaluate_low_coppersmith_completion_task,
                                        task,
                                    ): task
                                ...
                                completion_results.extend(wave_results)
                                if any(not result.get("hard_eligible") for result in wave_results):
                                    break
                        else:
                            with concurrent.futures.ProcessPoolExecutor(
                                ...
                                completion_results = list(
                                    executor.map(evaluate_low_coppersmith_completion_task, tasks)
                                )
                        consistent_completion_count = len(completion_results)
                        for completion_result in completion_results:
                            completion_status = str(completion_result.get("status"))
                            status_counts[completion_status] = status_counts.get(completion_status, 0) + 1
                            if completion_result.get("oracle_called"):
                            ...
                            if completion_result.get("factors"):
                                factors.extend(completion_result.get("factors") or [])
                            if completion_status == "factored":
                                event["low_coppersmith_minimization_factor"] = completion_result.get(
                                    "report", completion_result
                                )
                                print(json.dumps(event, sort_keys=True), flush=True)
                                print(
                        ...
                        for completion_value in range(completion_count):
                            ...
                            if completion_report.get("factors"):
                                factors.extend(completion_report.get("factors") or [])
                            if completion_status == "factored":
                                event["low_coppersmith_minimization_factor"] = completion_report
                                print(json.dumps(event, sort_keys=True), flush=True)
                                print(json.dumps({"event": "factored", **completion_report}, sort_keys=True), flush=True)
                                return 0

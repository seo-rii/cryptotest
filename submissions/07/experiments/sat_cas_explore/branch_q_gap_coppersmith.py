...
def item_fixed_ranges(item: dict[str, Any]) -> tuple[list[FixedRange], str | None]:
    ...
    cube_ranges = item.get("cube_ranges")
    if isinstance(cube_ranges, list):
        fixed_ranges = []
        for offset, raw_range in enumerate(cube_ranges):
            if not isinstance(raw_range, dict):
                return [], f"cube_ranges[{offset}] is not an object"
            try:
                fixed_ranges.append(
                    FixedRange(
                        int(raw_range["start"]),
                ...
                return [], f"failed to parse cube_ranges[{offset}]: {exc}"
        return fixed_ranges, None

    return [], "missing all_fix_p_range_args/all_fixed_ranges_text/cube_ranges"
...
def main() -> int:
    ...
    constants_path = ROOT / "src" / "investigate_rsa_partial_bits.py"

...
It mirrors the shape of `try_hensel_tail_cp_sat.py` without depending on
OR-Tools.
...
`../compare_tail_oracle.py`. The oracle run disables `tail_window_bits` and
...
```sh
/tmp/cryptotest_sat_venv/bin/python ../compare_tail_oracle.py \
...
```sh
/tmp/cryptotest_sat_venv/bin/python ../run_go_sat_filter.py \

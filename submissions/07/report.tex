...
다음 명령은 저장된 factor를 이용해 \(pq=N\), mask 일치,
...
g++ -O3 -std=c++17 \
  ...
  solve_grouped_hm_flatter.cpp \
  -L"$FLATTER_PREFIX/lib" -Wl,-rpath,"$FLATTER_PREFIX/lib" \
  -lflatter -lflint -lgmpxx -lgmp -lopenblas -pthread \
  -o /tmp/solve_grouped_hm_flatter

/tmp/solve_grouped_hm_flatter --modular-selftest

OPENBLAS_NUM_THREADS=1 /tmp/solve_grouped_hm_flatter \
...
\begin{lstlisting}
python3 run_grouped_hm_scan.py \
  /tmp/solve_grouped_hm_flatter \
  --output-dir /tmp/challenge-full-scan \
...
\subsection{하위 곱셈 exact-tail CP-SAT}
...
\subsection{Exact 다변수 Herrmann--May}
...
\subsection{얕은 grouped Herrmann--May}
...
\subsection{단변수 two-sided \texorpdfstring{\(p\)}{p} window}
...
\subsection{Residual partial-\texorpdfstring{\(p\)}{p} 격자}
...
\subsection{Folded \texorpdfstring{\(p/q\)}{p/q} Coron 격자}
...
\subsection{q-gap SAT learned clause}
...
\subsection{low600 \texorpdfstring{\(p\)}{p}-Coppersmith clause}
...
\subsection{SAT ranker와 다양성 샘플링}
...
\subsection{Jochemsz--May low-lift}

...
- [`src/plaintext.txt`](src/plaintext.txt): ASCII 평문
- [`src/plaintext.hex`](src/plaintext.hex): 마지막 줄바꿈을 제외한 평문의 hexadecimal 표현
...
복원한 factor, 유출 mask, RSA 재암호화는 다음 명령으로
독립 검증한다.
...
[`src/run_grouped_hm_scan.py`](src/run_grouped_hm_scan.py)다.

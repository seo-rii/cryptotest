...
`r0` 뒤에 가능한 하위 16비트를 붙여 `x=(r0<<16)|low16`을 만들고,
...
- `(x,+y)`와 `(x,-y)`는 각각 `R,-R`이고 `X(dR)=X(-dR)`이므로 한
  부호만 계산했다.
- 임의 점에 대한 `d`배는 width-5 wNAF를 사용해 addition 수를 줄였다.
- 반복되는 고정점 `Q`는
...
구현하고 Jacobian point를 48바이트 POD로 둔다. 고정 `Q` table은 한 번
...
`s2`, `r3`를 검증하고 median/MAD/raw sample을 보존했다.
...
native 최종 비교는 원본 Python/GMP/native를 같은 interleaved run에서
warm-up 1회 후 각각 5회 측정했다.
...
point/scalar 256개를 교차 검증했다. 모든 측정 process도 `P=dQ`, `d`,
`s2`, `r3`와 정답 low bits를 다시 검사했다.
...
- [Hamburg, *Faster Montgomery and double-add ladders for short Weierstrass curves*](https://eprint.iacr.org/2020/437): short-Weierstrass ladder 개선 후보.
